...
from {{ cookiecutter.project__module }}.config.environ import env

from {{ cookiecutter.project__module }}.cli import global_options
from {{ cookiecutter.project__module }}.signals import ({{ cookiecutter.project__module }}_version_upgraded,
                               cli_{{ cookiecutter.project__module }}_execute_command,
                               cli_{{ cookiecutter.project__module }}_upgrade_templates,)
from {{ cookiecutter.project__module }}.utils.reflect import fqn
...
def configure_beat():
    ...
    from {{ cookiecutter.project__module }}.engine.tasks.periodic import (
...
def upgrade(ctx, prompt, migrate, static, verbose, run_check,  # noqa: C901
    ...
    try:
        ...
        with atomic():
            ...
            if emails:
                from {{ cookiecutter.project__module }}.models import EmailTemplate, SystemEmailTemplate
                SystemEmailTemplate.objects.populate()

                from {{ cookiecutter.project__module }}.models import Content, Organization
                if verbose == 1:
