...
```shell
# Install Python binaries if not already present
uv python install 3.13  # or any other version required, see https://dev.azure.com/worldfoodprogramme/mVAM/_git/mvam-pipelines-v2?path=/.python-version
...
Start style.scss builder (with watch)
```bash
./manage.py tailwind start
```
and the usual runserver
```bash
