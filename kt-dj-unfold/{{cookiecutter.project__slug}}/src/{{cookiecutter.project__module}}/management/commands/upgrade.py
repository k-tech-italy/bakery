...
@click.option("--prompt/--no-input", default=True, is_flag=True, help="Do not prompt for parameters")
@click.option("--migrate/--no-migrate", default=True, is_flag=True, help="Run database migrations")
@click.option("--static/--no-static", default=False, is_flag=True, help="Collect static assets")
@click.option("--traceback", "-tb", default=False, is_flag=True, help="Raise on exceptions")
...
def command(
    ...
    **kwargs,
    ...
    try:
        extra = {"no_input": prompt, "verbosity": verbosity - 1}
