...
from {{ cookiecutter.project__module }} import SmartEnv
...
def missing_explicit(var):
    return Error(
        ...
        id="{{ cookiecutter.project__module }}.E001",
    )
...
def check_environment(app_configs: AppConfig, **kwargs: Any) -> list[CheckMessage]:
    ...
    instance = os.environ.get("{{ cookiecutter.project__module }}_INSTANCE", f"{settings}.env")
